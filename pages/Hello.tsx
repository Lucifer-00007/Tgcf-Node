...
const TelegramIcon = () => (
    <div className="relative w-24 h-24">
        ...
        <div className="absolute top-16 left-1/2 transform -translate-x-1/2 flex space-x-1">
            {[...Array(5)].map((_, i) => <div key={i} className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" style={{ animationDelay: `${i * 100}ms` }}></div>)}
            {[...Array(4)].map((_, i) => <div key={i+5} className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" style={{ animationDelay: `${(i+5) * 100}ms` }}></div>)}
        </div>
    </div>
);
    ...
    <div className="flex flex-col items-center justify-center h-full text-center">
      ...
      <div className="max-w-3xl w-full text-left">
        <CollapsibleSection title="Features">
          ...
          <p className="text-gray-600 mb-4">The key features are:</p>
